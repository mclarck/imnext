...
export default function useCart() {
  ...
  async function processPayment() {
    try {
      ...
      const transaction = {
        ...
        orders: _.map(cart?.items, (o) => ({
          quantity: o.quantity,
          stock: o.stock?.id,
        })),
      };
      const response = await rest.mutate(
        "POST",
        "/api/operations",
        transaction
      );
      if (response.ok) {
        ...
        alert(
          t("An error has occured during the transaction, please contact us")
        );
      }
    } catch (error) {
      handleError(error);
  ...
  function notifyTransaction(payload: any) {
    if (analytic) {
      analytic.emit("order", payload);
    }
  }
  ...
  return {
    ...
    isOrderSent,
    replace,
    loading: !isReady || loading || processing,
    company,
    isEmptyCart: _.isEmpty(cart?.items),
    cart: cart?.items,
    bills,
    processPayment,
    remove: onRemove,
